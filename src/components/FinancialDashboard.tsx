...
import { FinancialRecord, Patient, ProfessionalProfile } from '../types';
...
export const FinancialDashboard: React.FC<FinancialDashboardProps> = ({
  ...
  const [description, setDescription] = useState('Sessão de Psicoterapia Individual • Telemedicina');
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-6 rounded-3xl bg-[#120b24] border border-[#2a1b4e]">
        <div>
          ...
          <p className="text-xs text-purple-300/70">
            Controle de honorários, conciliação PIX instantânea e recibos médicos válidos.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        {/* Metric 3: Honorários Pendentes */}
        <div className="p-4 sm:p-5 rounded-2xl bg-[#120b24] border border-[#2a1b4e] space-y-2">
          <div className="flex items-center justify-between text-xs text-purple-300/80">
            <span>A Receber</span>
            <div className="p-1 rounded-lg bg-pink-950 text-[#ff007f]">
              <Clock className="w-3.5 h-3.5" />
            </div>
          </div>
          <div className="text-xl sm:text-2xl font-black text-[#ff007f]">
            R$ {pendingRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </div>
          <div className="text-[11px] text-pink-300/70 font-medium">
            Cobrança WhatsApp disponível
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-[#120b24] border border-[#2a1b4e] space-y-2">
          <div className="flex items-center justify-between text-xs text-purple-300/80">
            ...
            <div className="p-1 rounded-lg bg-[#bf5af2]/20 text-[#bf5af2]">
      ...
      {/* Filter and Table of Financial Records */}
      <div className="rounded-3xl bg-[#120b24] border border-[#2a1b4e] p-4 sm:p-6 space-y-4">
        
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-[#2a1b4e]">
          <h3 className="text-sm sm:text-base font-bold text-white flex items-center gap-2">
            <Receipt className="w-4 h-4 text-[#bf5af2]" />
            Extrato de Honorários & Recibos Emitidos
          </h3>
              ...
              <button
                ...
                className={`px-3 py-1 rounded-xl capitalize font-semibold transition-all ${
                  filterStatus === st
                    ...
                    : 'bg-[#0b0616] text-purple-300/70 hover:text-white border border-[#2a1b4e]'
                }`}
              >
                {st}
              </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-[#2a1b4e] text-purple-300/70">
                <th className="pb-3 font-semibold">Paciente & Descrição</th>
                <th className="pb-3 font-semibold">Data</th>
                <th className="pb-3 font-semibold">Método</th>
                <th className="pb-3 font-semibold">Valor</th>
                <th className="pb-3 font-semibold">Status</th>
                <th className="pb-3 font-semibold text-right">Recibo Clínico</th>
              </tr>
            </thead>
          ...
          <div className="w-full max-w-lg rounded-3xl bg-[#120b24] border border-[#bf5af2] p-6 shadow-2xl relative">
            ...
            <div className="p-6 sm:p-8 rounded-2xl bg-white text-slate-900 border border-slate-300 space-y-4 text-xs shadow-md printable-document">
              ...
              <div className="space-y-3 text-slate-800">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                  ...
                  <div className="text-slate-900 font-bold text-sm">
                    {profile === 'psicologo' ? 'Dra. Beatriz Albuquerque • CRP 06/148.920' : 'Dr. Rodrigo Vasconcelos • CRM 152.480-SP / RQE 78.432'}
                  </div>
                  <div className="text-[10px] text-slate-600">
                    CPF/CNPJ: 421.908.778-00 • Consultório PSICOOL
                  </div>
                </div>
              ...
              <div className="pt-6 text-center border-t border-slate-300">
                <div className="w-56 mx-auto border-b border-slate-900 mb-1" />
                <p className="font-bold text-slate-900 text-xs">
                  {profile === 'psicologo' ? 'Dra. Beatriz Albuquerque • CRP 06/148.920' : 'Dr. Rodrigo Vasconcelos • CRM 152.480-SP'}
                </p>
                <p className="text-[10px] text-slate-500">
                  Assinatura Digital com Validação Eletrônica PSICOOL
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-4 no-print">
              <button
                onClick={() => setSelectedReceipt(null)}
                className="flex-1 py-2.5 rounded-xl bg-[#1c1236] text-purple-300 font-bold text-xs hover:bg-[#251847]"
              >
                Fechar
              </button>
              <button
                onClick={() => window.print()}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#bf5af2] to-[#ff007f] text-white font-bold text-xs shadow-lg flex items-center justify-center gap-1.5 hover:brightness-110 active:scale-95 transition-all"
              >
          ...
          <div className="w-full max-w-md rounded-3xl bg-[#120b24] border border-[#bf5af2] p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateRecord} className="space-y-4 text-xs">
              <div>
                ...
                >
                  {patients.map((p) => (
                    <option key={p.id} value={p.name} className="bg-[#120b24]">
                      {p.name}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                >
                  ...
                  <option value="cartao" className="bg-[#120b24]">Cartão de Crédito / Mercado Pago</option>
                  <option value="convenio" className="bg-[#120b24]">Convênio / Reembolso</option>
                </select>
              </div>

              <div>
                <label className="text-purple-300 font-semibold block mb-1">Descrição</label>
                <input
                  type="text"
                  value={description}
